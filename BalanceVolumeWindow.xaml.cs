...
namespace InfovojnaRadio
{
	...
	public partial class BalanceVolumeWindow : Window
	{
		...
		{
			BalanceSliderToolTip = string.Format("Rovnováha {0}", Math.Truncate(sldBalance.Value));
		}
		...
		{
			VolumeSliderToolTip = string.Format("Hlasitosť {0}%", Math.Truncate(sldVolume.Value));
		}
